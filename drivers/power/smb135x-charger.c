				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY_LEVEL:
		...
		queue_delayed_work(system_power_efficient_wq, &chip->heartbeat_work,
			msecs_to_jiffies(0));
		break;
...
static void aicl_check_work(struct work_struct *work)
{
	...
	queue_delayed_work(system_power_efficient_wq, &chip->src_removal_work,
...
static int otg_oc_handler(struct smb135x_chg *chip, u8 rt_stat)
{
	...
	queue_delayed_work(system_power_efficient_wq, &chip->ocp_clear_work,
		msecs_to_jiffies(0));
